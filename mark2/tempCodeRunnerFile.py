...
def driver_login():
    ...
    bus = mongo.db.buses.find_one({'route_number': route_number, 'unique_code': unique_id})

    if bus:
        # Store driver info in session
        session['user_type'] = 'driver'
        session['user_id'] = str(bus['_id'])  # Storing bus ID or driver's info as required
        session['route_number'] = bus['route_number']
        session['driver_name'] = bus['driver_name']  # Storing driver name for later use
...
def generate_code(bus_id):
    ...
    mongo.db.buses.update_one({'_id': ObjectId(bus_id)}, {'$set': {'unique_code': unique_code}})

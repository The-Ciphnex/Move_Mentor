...
def login(user_type):
    if request.method == 'POST':
        if user_type == 'college':
            college = mongo.db.colleges.find_one({'email': request.form['email']})
            ...
            driver = mongo.db.drivers.find_one({
                'route_number': request.form['route_number'],
            ...
            student = mongo.db.students.find_one({'email': request.form['email']})
